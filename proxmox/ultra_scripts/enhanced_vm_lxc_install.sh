...
STORAGE=$(whiptail --menu "Select Storage:" 15 50 5 $(for s in $STORAGE_OPTIONS; do echo "$s Storage"; done) --default-item "$DEFAULT_STORAGE" 3>&1 1>&2 2>&3)
if [[ $? -ne 0 ]]; then exit 1; fi
...
if [[ "$CHOSEN_TYPE" == "LXC" ]]; then
    ...
    TEMPLATE=$(whiptail --title "Choose LXC Template" --menu "Select a $DISTRO template:" 15 60 6 $(for t in $TEMPLATE_LIST; do echo "$t" "Template"; done) 3>&1 1>&2 2>&3)
    if [[ $? -ne 0 ]]; then exit 1; fi
...
if [[ "$CHOSEN_TYPE" == "VM" ]]; then
    ...
    ISO=$(whiptail --menu "Select ISO Image:" 15 60 5 $(for i in $ISO_IMAGES; do echo "$i" "ISO"; done) --default-item "$DEFAULT_ISO" 3>&1 1>&2 2>&3)
    if [[ $? -ne 0 ]]; then exit 1; fi
